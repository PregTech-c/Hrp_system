...
from payroll.models import Position, EmployeeTitle, Nationality, Branch, ServiceLine
from payroll.models import EmployeeProfile, NextOfKin, EmployeeDocument, Dependants
...
class EmployeeSearchListForm(Form):
    emp_name = CharField(min_length=1)
...
class NumberOfChildrenForm(ModelForm):
    class Meta:
        model = Position
        exclude = ()
        widgets = {
...
class StatusForm(ModelForm):
    ...
    class Meta:
        ...
        exclude = ()
        widgets = {
            'status_comment': TextInput(attrs={'size': '60'}),
        }
...
class DependantsForm(ModelForm):

    def __init__(self, *args, **kwargs):
        super(DependantsForm, self).__init__(*args, **kwargs)
        self.error_class = DivErrorList
        self.fields['gender'].empty_label = '--Select--'
        self.fields['relationship'].empty_label = '--Select--'
    class Meta:
        model = Dependants
        exclude = ()
        widgets = {
            'full_name': TextInput(attrs={'size': '60'}),
            'nin': TextInput(attrs={'size': '60'}),
            'contact': TextInput(attrs={'size': '60'}),
...
class EmployeeProfileForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'passport_number': TextInput(attrs={'size': '60'}),
            'number_of_children': TextInput(attrs={'size': '60'}),
            'NIN': TextInput(attrs={'size': '60'}),
        #    'contract_type': ModelChoiceField(queryset = EmployeeProfile.objects.all()),
            'driving_license': TextInput(attrs={'size': '60'}),
            ...
            'date_of_birth': JQueryUIDatepickerWidget,
            'national_id_expiry': JQueryUIDatepickerWidget,
            'passport_expiry_date': JQueryUIDatepickerWidget,
            'license_expiry_date': JQueryUIDatepickerWidget
