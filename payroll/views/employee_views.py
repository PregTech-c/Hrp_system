...
from payroll.models import User, EmployeeProfile, EmployeeDocument

from payroll.forms import (EmployeeForm, EmployeeProfileForm,
    ContractualPayForm, EmployeeSearchForm, EmployeeSearchListForm,
    NextOfKinForm, EmployeeDocumentForm, EmployeePhotoForm, EmployeeTitleForm,
    PositionForm)
from payroll.forms.qualification_forms import (EducationLevelForm,
    CertificationForm, MembershipForm, SkillForm)
...
class UploadEmployeePhotoView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            rec=EmployeeProfile.objects.get(
                pk=form.cleaned_data['employee_profile_id']).user
            notify.send(
                None,
                recipient=rec,
                actor=request.user,
                actor_url=reverse('payroll:user_detail',
                    kwargs={'pk': kwargs.get('pk')}), verb='Your new profile photo.',
                actor_text=request.user.profile.get_fullname(),
                nf_type='added_photo'
            )
...
class UploadEmployeeDocumentView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            rec=form.cleaned_data['employee_profile'].user
            notify.send(
                None,
                recipient=rec,
                actor=request.user,
                ...
                verb='{} added a document to your profile.'.format(
                    request.user.profile.get_fullname()),
                actor_text=request.user.profile.get_fullname(),
                nf_type='added_document'
            )
...
#            employee = User.objects.get(pk=1)
        ...
        return render(request,
                    self.template_name,
                    {
                        'employee_title_form': EmployeeTitleForm,
                        'employee_education_form': EducationLevelForm,
...
#            s=s
        ...
        if uform.is_valid() and pform.is_valid() \
            and sform.is_valid() and nkinform.is_valid():
            user = uform.save()
            uform.save_m2m()
            nkin=nkinform.save()
            ...
            if edit:
                prev_salary = 0
                pay = user.profile.get_contractual_pay()
                if pay is not None:
                    prev_salary = pay.amount

                if prev_salary != sform.cleaned_data['amount']:
                    rec=user
                    notify.send(
                        None,
                        recipient=rec,
                        ...
                        actor_url=reverse('payroll:user_detail',
                            kwargs={'pk': kwargs.get('pk')}),
                        verb='{} adjusted your salary to {}.'.format(
                            request.user.profile.get_fullname(),
                            salary.amount),
                        actor_text=request.user.profile.get_fullname(),
                        nf_type='adjusted_salary'
                    )
