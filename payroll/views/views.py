...
def home(request):
    ...
    if profile:
        ...
        leave_request = profile.get_leave_approval(status='approved')
        if status.leave_approved:
            for leave in leave_approved:
                notifications.append(
                    'Received <a href="{}">Appraisal</a> ({}) from {}'.format(
                    reverse('leave:received_leave_measure_list',
                        kwargs={'emp_id': emp.id}),
                        emp_leave.leave.description,
                        emp_leave.employee_profile.get_fullname()
                    )
                )
